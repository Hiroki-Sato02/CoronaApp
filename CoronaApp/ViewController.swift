...
class ViewController: UIViewController {
    ...
    func setUpAPI(parentView: UIView) {
        ...
        setUpAPILabel(dischage, size: size, centerX: rightX, y: 260, font: font, color: color, parentView)
        ...
        CovidAPI.getTotal(completion: {(result: CovidInfo.Total) -> Void in
            DispatchQueue.main.async {
                ...
                dischage.text = "\(result.discharge)"
                HUD.hide()
            }
        })
    }
    ...
    func setUpImageButton(_ name: String, x: CGFloat) -> UIButton {
        ...
        button.frame.origin = CGPoint(x: x, y: 45)
